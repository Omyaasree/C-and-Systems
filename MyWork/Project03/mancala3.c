...
#define DEFAULT_PORT 12344

// Structure to hold client socket details
struct Client {
    int sockFd;
};
...
// coed from the client Demo code 
...
// if the user ever had empty pockets which mean sthey have the chance to steal BLOCK
...
// Function to defend or make the largest possible move for the computer
int defense_or_biggest_move(int board[]) {
    ...
    // If no blocking move found, make the biggest move available
    int max = 0;
    int best = 7;
    for (int i = 7; i <= 12; i++) {
        if (board[i] > max) {
