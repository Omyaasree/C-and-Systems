...
// // Send one message: a single byte and message itself (including \0)
// void doWrite(const char *msg) {
//     unsigned char len = strlen(msg) + 1;
...
void handleServerCommand(const char *cmd, int board[], int sockFd) {
    if (strcmp(cmd, "LOGIN") == 0) {
        ...
        char playMsg[3]; 
        snprintf(playMsg, sizeof(playMsg), "%d", move + 1);
        ...
        // int move = cmd[4] - '0';
        // move(board, move, 1); // Assume opponent is player 1
